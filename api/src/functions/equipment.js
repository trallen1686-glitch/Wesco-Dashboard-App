...
app.http("listEquipment", {
  methods: ["GET"],
  authLevel: "anonymous",
  route: "equipment",
  handler: async (request, context) => {
    try {
      const sheetsResp = await graphFetch("/worksheets");
      ...
      const equipment = await Promise.all(
        sheets.map(async (sheet) => {
          ...
          const [machineRange, headerRange] = await Promise.all([
            graphFetch(`/${seg}/range(address='B3')`),
            graphFetch(`/${seg}/range(address='G3:H12')`),
          ]);
          ...
          return {
            ...
            lastLocation: fields["Last Location"] || "",
...
app.http("addEquipment", {
  ...
  handler: async (request, context) => {
    try {
      ...
      const beforeResp = await graphFetch("/worksheets");
      const beforeNames = new Set(beforeResp.value.map((s) => s.name));

      await graphFetch("/worksheets('Template')/copy", {
        method: "POST",
        body: JSON.stringify({ positionType: "End" }),
      });

      const afterResp = await graphFetch("/worksheets");
      ...
      await graphFetch(`/${worksheetSegment(newSheet.name)}`, {
        method: "PATCH",
      ...
      await Promise.all([
        ...
        graphFetch(`/${seg}/range(address='H4:H12')`, {
          method: "PATCH",
          body: JSON.stringify({
            values: [
              [iso(today)],
